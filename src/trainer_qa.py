...
class PolicyTrainer(Trainer):
    ...
    def measure_generation(retrieved_list, answer_list):
        ...
        recall = sum([1 for ans in answer_list if (ans in all_text) ]) / n_short_answers
        return recall
    ...
    def compute_loss_reward(
        ...
        for i, key in enumerate([int(k) for k in range(query.size()[0])]):
            try: 
                ...
                reward = self.measure_generation(candidate, truth[i])
    ...
    def compute_loss(
        ...
        qrels = [self.train_dataset.answers[id] for id in ids]
