            ...
            with torch.no_grad():
                ...
                with unwrap_model_for_generation(model, self.accelerator) as unwrapped_model:
                    for i in range(0, queries.shape[0], args.local_rollout_forward_batch_size):
                        ...
                        query_target, target_length = get_expected_inputs(query, target_text, tokenizer)
                        target = query_target[:, -target_length:]
                        value_output = forward(ref_policy, query_target, tokenizer.pad_token_id) # ref_policy == policy
                        ...
                        sequence_length_target = first_true_indices(target == tokenizer.pad_token_id) - 1

                        # [NOTE] here the example used a hf model to do rewarding
                        query_response_texts  = [tokenizer.decode(q_and_r) for q_and_r in postprocessed_query_response]
                        score = reward_model.calculate_rewards(query_response_texts, target_text)
                        ...
                        sequence_lengths_target.append(sequence_length_target)
                        scores.append(score)
                ...
                sequence_lengths_target = torch.cat(sequence_lengths_target, 0)
                ...
                ## [NOTE] made this for value
                sequence_lengths_p1_target = sequence_lengths_target + 1
                target_idxs = torch.arange(targets.shape[1], device=targets.device).repeat(targets.shape[0], 1)
                padding_mask_target = target_idxs > sequence_lengths_target.unsqueeze(1)
                padding_mask_p1_target = target_idxs > (sequence_lengths_p1_target.unsqueeze(1))
                values = torch.masked_fill(values, padding_mask_p1_target, 0)
                ...
                actual_end = torch.where(sequence_lengths_p1 < rewards.size(1), sequence_lengths_p1, sequence_lengths)
                rewards[[actual_start, actual_end]] += scores.to(rewards.device)
