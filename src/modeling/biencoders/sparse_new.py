...
class AttentionHead(nn.Module):
    def __init__(self, opt, encoder):
        ...
        self.args = opt
        if opt.zero_init:
            self.attn_layer.q_proj.weight.data.zero_()
            self.attn_layer.q_proj.bias.data.zero_()
            self.attn_layer.k_proj.weight.data.zero_()
            self.attn_layer.k_proj.bias.data.zero_()
...
class SparseAdaptiveEncoders(nn.Module):
    def __init__(
        ...
        self.sig_relu = lambda x: F.relu(x) * F.sigmoid(x)
    ...
    def forward(self, q_tokens, q_masks, prev_out, d_tokens=None, d_masks=None, **kwargs):
        ...
        if d_tokens is not None:
            ...
            print('q0', scores_0.softmax(-1).diag().tolist())
